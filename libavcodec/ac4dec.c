...
static void get_tab_border(int *atsg_sig, int num_aspx_timeslots, int num_atsg)
{
    ...
    case 6:
        memcpy(atsg_sig, tab_border[0][num_atsg >> 1], 5 * sizeof(int));
        break;
    case 8:
        memcpy(atsg_sig, tab_border[1][num_atsg >> 1], 5 * sizeof(int));
        break;
    case 12:
        memcpy(atsg_sig, tab_border[2][num_atsg >> 1], 5 * sizeof(int));
        break;
    case 15:
        memcpy(atsg_sig, tab_border[3][num_atsg >> 1], 5 * sizeof(int));
        break;
    case 16:
        memcpy(atsg_sig, tab_border[4][num_atsg >> 1], 5 * sizeof(int));
        break;
    default:
        av_assert0(0);
    }
                        ...
                        SubstreamChannel *ssch)
{
    ...
    ssch->scp.num_window_groups = 1;
    ssch->scp.transf_length_idx[0] = 3;
    ...
    for (int atsg = 0; atsg < ssch->aspx_num_noise; atsg++) {
        ...
        for (int sbg = 0; sbg < ssch->num_sbg_noise; sbg++) {
            ...
            if (ssch->aspx_noise_delta_dir[atsg] == 0) { /* FREQ */
                ...
                if (atsg == 0) {
                    ssch->qscf_noise_sbg[atsg][sbg]  = ssch->qscf_prev[ssch->aspx_num_noise_prev-1][sbg];
                    ssch->qscf_noise_sbg[atsg][sbg] += delta * ssch->aspx_data[1][atsg][sbg];
                } else {
